...
The following figures compare the JASPAR Motifs to the most predicivtive kmer (determined by having the largest absolute feature weight in the svm). We can see that the prediction of USF1 by our model has the highest consistency with the JASPAR motifs compared to the other two, which partially validate the prediction accuracy of our model. JASPAR motifs are acquired based on certain data set, and it is possible that our model provides a more accurate results for the proteins and cell lines we selected. 
...
The following three plots display the distribution of feature weights for each svm that was fitted. All the histograms are unimodal and centered around zero. The histograms show that a majority of the features provide little information towards predicting binding sites.
...
## Conclusions 
Based on the results shown above, we are able to draw some conclusions. First, we shoud admit that we could not reproduce all the results in Arvey et al because we only focus on three proteins and four cell lines while they did 286 experiments. We also used a different kernel for the SVM function which is available in the Kebabs package but we could produce similar results to the paper. We cold see that our SVM modeling of protein binding was extremely accurate as the AUC values are above 90% for all the combinations of proteins and cell lines. The AUC was uniformly high within cell-lines, and extremely high when combining data across cell lines, which is even higher than each individual combination. Similar to Arvey et al., we did see that some proteins are easier to predict than others, or to say we are able to predict USF1 binding more easily which has overall higher AUC values than JUND, MAX. Also similar to Arvey, we saw variation in predictive ability among cell-lines within the same protein. For instance, predictions of JUND binding in K652 were more accurate than in H1hesc. Patterns did not hold across proteins such as the predictions for MAX were more accurate in H1hesc than K652. These findings indicates that there may be other factors influencing the cell specific binding where one potential factor was the open chromatin as suggested by the paper. We would have liked to use open chromatin as a predictor in our svm model to see if we could get a better fit. However, the function we used to create out model did not allow for covariates, which may affect the prediction accuracy of our model. Though the limitations, we may still conclude that given a protein with unknown binding motif, support vector machine modeling may be a useful tool in predicting its binding site and achieve an acceptable prediction accuracy.  
